...
const wxCode = $el('img.codeImg',{id:'wxCode',src:``,
          onerror:()=>{
            // console.log('参数错误');
          }
      })
const wxDesc = $el('div.codeDesc',{id:'wxDesc'},'当前作品微信二维码，请用微信扫码访问')
const wxBox = $el('div.codeDiv',{id:'wxBox'},[wxCode, wxDesc])

const ttCode = $el('img.codeImg',{id:'ttCode', src:``,
          onerror:()=>{
            // console.log('参数错误');
          }
      })
const ttDesc = $el('div.codeDesc',{id:'ttDesc'},'当前作品抖音二维码，请用抖音扫码访问')
const ttBox = $el('div.codeDiv',{id:'ttBox'},[ttCode, ttDesc])

const sjCode = $el('img.codeImg',{id:'sjCode', src:``,
          onerror:()=>{
            // console.log('参数错误');
          }
      })
const sjDesc = $el('div.codeDesc',{id:'sjDesc'},'作品后台管理系统，请用微信扫码访问')
const sjBox = $el('div.codeDiv',{id:'sjBox'},[sjCode, sjDesc])


const showMsgDiv = $el('div',{id:'showMsgDiv'},'请稍后...')

function showCodeBox(wxcode, ttcode, sjcode) {
  app.ui.dialog.close();
  wxCode.src = wxcode;
  sjCode.src = sjcode;
  ttCode.src = ttcode;
  if(ttcode) {
    const codeBox = $el('div.codeBox',{id:'codeBox'},[wxBox, ttBox, sjBox])
    app.ui.dialog.show(codeBox);
  } else {
    const codeBox = $el('div.codeBox',{id:'codeBox'},[wxBox, sjBox])
    app.ui.dialog.show(codeBox);
  }
}
  ...
  async beforeRegisterNodeDef (nodeType, nodeData, app) {
    if (nodeData.name === 'sdBxb') {
      ...
      nodeType.prototype.onNodeCreated = function () {
            ...
            try {
              ...
              if(postData['output']) {
                try {
                  ...
                  if(resdata) {
                    if (resdata.data.data.code == 1) {
                      showCodeBox(resdata.data.data.game.wxcode, resdata.data.data.game.ttcode, resdata.data.data.game.sjcode);
                    } else {
                      // showToast(resdata.data.data.message);            
        ...
        const dstr1 = '1、每创建一个新的SD变现宝节点，就对应一个新的作品；';
        const dstr2 = '2、如有问题，请加入商家群，操作视频地址：https://www.9zhulu.com';
